<template>
  <div :class="['mx-auto transition-all duration-300 w-full', isSidebarCollapsed ? 'max-w-full px-3' : 'max-w-7xl px-3 lg:px-0']">
    ...
    <div class="flex flex-nowrap sm:flex-wrap gap-1 sm:gap-1.5 md:gap-2 lg:gap-3 mb-2 sm:mb-3">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-0.5 sm:p-1.5 md:p-2 lg:p-3 flex-1 min-w-0 min-h-[60px] sm:min-h-[70px] md:min-h-[80px]">
        <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between">
          <div class="order-2 sm:order-1">
            <h3 class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium mb-0.5 sm:mb-1 md:mb-2">{{ t('totalGraduatedStudents') }}</h3>
            <p class="text-base sm:text-base md:text-xl lg:text-2xl xl:text-3xl font-bold leading-tight sm:leading-normal text-gray-800 dark:text-white">{{ totalGraduatedStudents }}</p>
          </div>
      ...
      <!-- Active Graduated Card -->
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-0.5 sm:p-1.5 md:p-2 lg:p-3 flex-1 min-w-0 min-h-[60px] sm:min-h-[70px] md:min-h-[80px]">
        <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between">
          <div class="order-2 sm:order-1">
            <h3 class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium mb-0.5 sm:mb-1 md:mb-2">{{ t('activeGraduated') }}</h3>
            <p class="text-base sm:text-base md:text-xl lg:text-2xl xl:text-3xl font-bold leading-tight sm:leading-normal text-gray-800 dark:text-white">{{ activeGraduated }}</p>
          </div>
      ...
      <!-- Total Courses Completed Card -->
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-0.5 sm:p-1.5 md:p-2 lg:p-3 flex-1 min-w-0 min-h-[60px] sm:min-h-[70px] md:min-h-[80px]">
        <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between">
          <div class="order-2 sm:order-1">
            <h3 class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium mb-0.5 sm:mb-1 md:mb-2">{{ t('totalCoursesCompleted') }}</h3>
            <p class="text-base sm:text-base md:text-xl lg:text-2xl xl:text-3xl font-bold leading-tight sm:leading-normal text-gray-800 dark:text-white">{{ totalCoursesCompleted }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-0.5 sm:p-1.5 md:p-2 lg:p-3 flex-1 min-w-0 min-h-[60px] sm:min-h-[70px] md:min-h-[80px]">
        <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between">
          <div class="order-2 sm:order-1">
            <h3 class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium mb-0.5 sm:mb-1 md:mb-2">{{ t('totalProvinces') }}</h3>
            <p class="text-base sm:text-base md:text-xl lg:text-2xl xl:text-3xl font-bold leading-tight sm:leading-normal text-gray-800 dark:text-white">{{ totalProvinces }}</p>
          </div>
    ...
    <div class="max-h-[500px] overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-sm">
      <table class="w-full">
        ...
        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <!-- Graduated Student Rows -->
          <tr
            v-for="(student, index) in filteredGraduatedStudents"
            :key="student.id"
    ...
    <!-- View Student Courses Drawer -->
    <Transition name="drawer">
      <div
        v-if="showViewDrawer"
        class="fixed inset-0 bg-black bg-opacity-50 z-50"
        @click.self="showViewDrawer = false"
      >
        <div class="absolute right-0 top-0 h-full w-full max-w-md bg-white dark:bg-gray-800 shadow-xl overflow-y-auto">
          ...
          <div v-if="selectedStudent" class="p-6 space-y-4">
            ...
            <div class="pb-4 border-b border-gray-200 dark:border-gray-700">
              ...
              <div class="flex items-center gap-3">
                <div class="w-12 h-12 rounded-full flex items-center justify-center bg-blue-100 dark:bg-blue-900">
                  <span class="text-blue-600 dark:text-blue-400 font-semibold text-sm">{{ getInitials(selectedStudent.student.name) }}</span>
                </div>
                <div>
                  <h4 class="text-base font-semibold text-gray-900 dark:text-white">{{ selectedStudent.student.name }}</h4>
                  <p class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">{{ selectedStudent.student.id }}</p>
                </div>
              </div>
              <div class="grid grid-cols-2 gap-2 mt-3 text-xs">
                <div>
                  <span class="text-gray-500 dark:text-gray-400">{{ t('contact') }}:</span>
                  <span class="text-gray-900 dark:text-white ml-1">{{ selectedStudent.student.contact || 'N/A' }}</span>
                </div>
                <div>
                  <span class="text-gray-500 dark:text-gray-400">{{ t('province') }}:</span>
                  <span class="text-gray-900 dark:text-white ml-1">{{ selectedStudent.student.province || 'N/A' }}</span>
                </div>
              </div>
            </div>
            ...
            <!-- Courses Table -->
            <div>
              <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase mb-3">{{ t('courses') }}</h3>
                ...
                <table class="w-full">
                  <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                      ...
                      <th class="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">{{ t('courseName') }}</th>
                      <th class="px-3 py-2 text-center text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase"></th>
                    </tr>
                  </thead>
                  <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    <tr v-if="filteredCourses.length === 0">
                      <td colspan="3" class="px-3 py-4 text-center text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">
                        {{ t('noCourses') }}
                      </td>
                    </tr>
                    <tr
                    ...
                    >
                      ...
                      <td class="px-3 py-2 text-center">
                        <button
                          @click="deleteCourse(selectedStudent, index)"
              ...
              <div class="mt-3">
                <button
                  ...
                  class="w-full px-4 py-2 bg-blue-600 text-white rounded-sm hover:bg-blue-700 transition-colors font-medium text-sm flex items-center justify-center gap-2"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div
      ...
      @click.self="showAddCourseDialog = false"
    >
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow-xl p-6 max-w-md w-full mx-4">
...
// Load graduated students from JSON file or localStorage, then merge with student data
const loadGraduatedStudents = async () => {
  try {
    ...
    graduatedStudents.value = graduatedStudentsData.map(gs => {
    ...
    if (saved) {
      graduatedStudents.value = JSON.parse(saved)
    } else {
      graduatedStudents.value = []
    }
  }
}

// Save graduated students to localStorage
const saveGraduatedStudents = () => {
  try {
    localStorage.setItem('graduated_students_data', JSON.stringify(graduatedStudents.value))
  } catch (error) {
    console.error('Error saving graduated students:', error)
  }
}
...
const filteredGraduatedStudents = computed(() => {
  ...
  if (filterDateFrom.value || filterDateTo.value) {
    filtered = filtered.filter(gs => {
      if (!gs.graduatedDate) return false
      
      const graduatedDate = new Date(gs.graduatedDate)
      graduatedDate.setHours(0, 0, 0, 0)
      ...
      if (filterDateFrom.value && filterDateTo.value) {
        ...
        return graduatedDate >= fromDate && graduatedDate <= toDate
      } else if (filterDateFrom.value) {
        const fromDate = new Date(filterDateFrom.value)
        fromDate.setHours(0, 0, 0, 0)
        return graduatedDate >= fromDate
      } else if (filterDateTo.value) {
        const toDate = new Date(filterDateTo.value)
        toDate.setHours(23, 59, 59, 999)
        return graduatedDate <= toDate
      }
      return true
    })
  }
...
const filteredCourses = computed(() => {
  ...
  const courses = selectedStudent.value.courses || []
...
const totalGraduatedStudents = computed(() => graduatedStudents.value.length)

const activeGraduated = computed(() => {
  return graduatedStudents.value.filter(gs => gs.status === 'active').length
})

const totalCoursesCompleted = computed(() => {
  const allCourses = new Set()
  graduatedStudents.value.forEach(gs => {
    gs.courses.forEach(course => allCourses.add(course))
  })
...
const totalProvinces = computed(() => {
  ...
  graduatedStudents.value.forEach(gs => {
    if (gs.student.province) {
      provinces.add(gs.student.province)
    }
  })
...
const confirmAddCourse = async () => {
  ...
  try {
    ...
    handleError(err, { userMessage: 'Failed to add course. Please try again.' })
  }
}
...
const deleteCourse = async (student, courseIndex) => {
  try {
    ...
    handleError(err, { userMessage: 'Failed to delete course. Please try again.' })
  }
}
...
onMounted(() => {
  ...
  window.addEventListener('storage', (e) => {
    if (e.key === 'graduated_students_data') {
      loadGraduatedStudents()
    }
