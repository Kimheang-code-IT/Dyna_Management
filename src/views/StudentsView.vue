  ...
  <div :class="['mx-auto transition-all duration-300 w-full', isSidebarCollapsed ? 'max-w-full px-3' : 'max-w-7xl px-3 lg:px-0']">
    ...
    <div class="flex flex-nowrap sm:flex-wrap gap-1 sm:gap-1.5 md:gap-2 lg:gap-3 mb-2 sm:mb-3">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-0.5 sm:p-1.5 md:p-2 lg:p-3 flex-1 min-w-0 min-h-[60px] sm:min-h-[70px] md:min-h-[80px]">
        <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between">
          <div class="order-2 sm:order-1">
            <h3 class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium mb-0.5 sm:mb-1 md:mb-2">{{ t('male') }}</h3>
            <p class="text-base sm:text-base md:text-xl lg:text-2xl xl:text-3xl font-bold leading-tight sm:leading-normal text-gray-800 dark:text-white">{{ maleCount }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-0.5 sm:p-1.5 md:p-2 lg:p-3 flex-1 min-w-0 min-h-[60px] sm:min-h-[70px] md:min-h-[80px]">
        <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between">
          <div class="order-2 sm:order-1">
            <h3 class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium mb-0.5 sm:mb-1 md:mb-2">{{ t('female') }}</h3>
            <p class="text-base sm:text-base md:text-xl lg:text-2xl xl:text-3xl font-bold leading-tight sm:leading-normal text-gray-800 dark:text-white">{{ femaleCount }}</p>
          </div>
          <div class="w-10 h-10 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 bg-pink-100 dark:bg-pink-900/30 rounded-full flex-shrink-0 flex items-center justify-center order-1 sm:order-2 mb-2 sm:mb-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 sm:h-4 sm:w-4 md:h-5 md:w-5 lg:h-6 lg:w-6 text-pink-600 dark:text-pink-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
            </svg>
          </div>
        </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-0.5 sm:p-1.5 md:p-2 lg:p-3 flex-1 min-w-0 min-h-[60px] sm:min-h-[70px] md:min-h-[80px]">
        <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between">
          <div class="order-2 sm:order-1">
            <h3 class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium mb-0.5 sm:mb-1 md:mb-2 break-words line-clamp-2">{{ t('topCourse') }}</h3>
            <p class="text-base sm:text-base md:text-xs lg:text-md xl:text-xl font-bold leading-tight sm:leading-normal text-gray-800 dark:text-white">{{ topCourse.name }}</p>
            <p class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-1">{{ topCourse.count }} {{ t('students') }}</p>
          </div>
        ...
        <table class="w-full">
          ...
          <thead class="bg-gray-50 dark:bg-gray-700 sticky top-0 z-10">
            <tr>
              ...
              <th class="py-3 text-left px-2 text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('gender') }}</th>
              <th class="py-3 px-6 text-left text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('course') }}</th>
              <th class="py-3 text-left px-5 text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('session') }}</th>
              <th class="py-3 text-left px-5 text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('province') }}</th>
              <th class="py-3 text-left px-4 text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('contact') }}</th>
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <!-- No Results Found -->
            <tr v-if="filteredStudents.length === 0 && (searchQuery || selectedCourse || selectedProvince || filterDateFrom || filterDateTo)">
            ...
            >
              ...
              <td class="px-3 py-3 text-xs">
                <div class="flex items-center gap-2">
                  <div :class="['w-10 h-10 rounded-md flex items-center justify-center overflow-hidden flex-shrink-0 relative', (!student.profileImage || hasImageFailed(student.id)) ? 'border border-gray-300 dark:border-gray-600 bg-blue-100 dark:bg-blue-900' : '']">
                    <img 
                      v-if="student.profileImage && !hasImageFailed(student.id)" 
                      :src="student.profileImage" 
                      :alt="student.name || student.nameEnglish"
                  ...
                  <div>
                    <div class="font-medium text-gray-900 dark:text-white text-xs">{{ student.name || student.nameEnglish }}</div>
                    <div class="text-[10px] text-gray-500 dark:text-gray-400">{{ student.id }}</div>
                    <div v-if="student.nameKhmer" class="text-[10px] text-gray-500 dark:text-gray-400">{{ student.nameKhmer }}</div>
                  </div>
                </div>
              </td>
              ...
              <td class="px-2 py-3 whitespace-nowrap">
                <span class="px-2 py-1 text-[10px] font-medium rounded-full bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200">
                  {{ student.course }}
                </span>
              </td>
              ...
              <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-700 dark:text-gray-300">{{ student.province }}</td>
              <td class="px-4 py-3 text-xs text-gray-700 dark:text-gray-300">
                ...
                <div v-if="student.telegram || student.telegramContact" class="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5">
            ...
            <div>
              ...
              <div class="relative">
                ...
                >
                  ...
                  <div v-if="form.profileImage" class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-300 flex items-center justify-center">
                    <div class="opacity-0 group-hover:opacity-100 px-3 py-2 bg-blue-600 text-white rounded-md transition-all duration-300 font-medium text-sm shadow-lg flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow-xl p-6 max-w-md w-full mx-4">
        <div class="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ t('areYouSure') }} {{ t('delete') }} {{ t('student') }}?</p>
          </div>
        </div>
        <p class="text-sm text-gray-700 dark:text-gray-300 mb-6">
          {{ t('student') }}: <span class="font-semibold">{{ studentToDelete?.name || studentToDelete?.nameEnglish }}</span>
        </p>
