<template>
  <div
        ...
        <div
          ...
          <div class="order-2 sm:order-1">
            ...
            <p
              ...
              {{ totalHistoryEntries }}</p>
            <p class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-2">{{ filteredHistory.length }} {{
              t('entries') }}</p>
          </div>
          <div
      ...
      <div
        ...
        <div
          ...
          <div class="order-2 sm:order-1">
            <h3
              class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium mb-0.5 sm:mb-1 md:mb-2">
              {{ t('mostCommonAction') }}</h3>
            <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ mostCommonAction.name || '-' }}</p>
            <p class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-2">{{ mostCommonAction.count || 0 }} {{
              t('activities') }}</p>
          </div>
          <div
      ...
      <div
        ...
        <div
          ...
          <div class="order-2 sm:order-1">
            ...
            <p class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-2">{{ mostActiveUser.count || 0 }} {{
              t('activities') }}</p>
          </div>
          <div
          ...
          <div class="flex items-center gap-3 flex-1 justify-end">
            ...
            <div class="relative">
              <select v-model="filterAction"
                ...
                <option value="print">{{ t('print') || 'Print' }}</option>
                <option value="downloadInvoice">{{ t('downloadInvoice') || 'Download Invoice' }}</option>
              </select>
              <div class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                <svg class="h-5 w-5 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor"
                  viewBox="0 0 24 24">
        ...
        <table class="w-full">
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="(entry, index) in filteredHistory" :key="entry.id"
              ...
              <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-700 dark:text-gray-300">{{
                formatCreatedDate(entry.timestamp) }}</td>
    ...
    <ConfirmDialog :model-value="showDeleteDialog" :title="t('deleteHistoryEntry')"
      :message="t('confirmDeleteHistoryEntry')" :confirm-label="t('delete')" :cancel-label="t('cancel')" type="danger"
      confirm-button-class="bg-red-600 hover:bg-red-700" @confirm="confirmDelete" @cancel="showDeleteDialog = false"
...
import { getHistory, filterHistory, formatTimestamp, deleteHistoryEntry } from '../utils/history'
...
const getActionClass = (action) => {
  const classes = {
    ...
    'downloadInvoice': 'bg-cyan-100 text-cyan-800'
  }
  ...
  try {
    await withLoading(async () => {
      if (entryToDelete.value && deleteHistoryEntry(entryToDelete.value.id)) {
        ...
        successMessageTitle.value = t('historyEntryDeleted') || 'History Entry Deleted'
        successMessageText.value = t('historyEntryDeleted') || 'History entry has been deleted successfully!'
        setTimeout(() => {
        ...
        errorMessageText.value = t('failedToDeleteHistoryEntry') || 'Failed to delete history entry'
        setTimeout(() => {
