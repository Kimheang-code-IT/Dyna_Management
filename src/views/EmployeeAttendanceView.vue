  ...
  <div :class="['mx-auto transition-all duration-300 w-full', isSidebarCollapsed ? 'max-w-full px-3' : 'max-w-7xl px-3 lg:px-0']">
    ...
    <div class="flex flex-nowrap sm:flex-wrap gap-1 sm:gap-1.5 md:gap-2 lg:gap-3 mb-2 sm:mb-3">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-0.5 sm:p-1.5 md:p-2 lg:p-3 flex-1 min-w-0 min-h-[60px] sm:min-h-[70px] md:min-h-[80px]">
        <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between">
          <div class="order-2 sm:order-1">
            <h3 class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium mb-0.5 sm:mb-1 md:mb-2">{{ t('totalAttendanceRecords') }}</h3>
            <p class="text-base sm:text-base md:text-xl lg:text-2xl xl:text-3xl font-bold leading-tight sm:leading-normal text-gray-800 dark:text-white">{{ totalRecords }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-0.5 sm:p-1.5 md:p-2 lg:p-3 flex-1 min-w-0 min-h-[60px] sm:min-h-[70px] md:min-h-[80px]">
        <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between">
          <div class="order-2 sm:order-1">
            <h3 class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium mb-0.5 sm:mb-1 md:mb-2">{{ t('approvedRecords') }}</h3>
            <p class="text-base sm:text-base md:text-xl lg:text-2xl xl:text-3xl font-bold leading-tight sm:leading-normal text-gray-800 dark:text-white">{{ approvedRecords }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-0.5 sm:p-1.5 md:p-2 lg:p-3 flex-1 min-w-0 min-h-[60px] sm:min-h-[70px] md:min-h-[80px]">
        <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between">
          <div class="order-2 sm:order-1">
            <h3 class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium mb-0.5 sm:mb-1 md:mb-2">{{ t('pendingRecords') }}</h3>
            <p class="text-base sm:text-base md:text-xl lg:text-2xl xl:text-3xl font-bold leading-tight sm:leading-normal text-gray-800 dark:text-white">{{ pendingRecords }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-sm shadow p-0.5 sm:p-1.5 md:p-2 lg:p-3 flex-1 min-w-0 min-h-[60px] sm:min-h-[70px] md:min-h-[80px]">
        <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between">
          <div class="order-2 sm:order-1">
            <h3 class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium mb-0.5 sm:mb-1 md:mb-2">{{ t('totalHours') }}</h3>
            <p class="text-base sm:text-base md:text-xl lg:text-2xl xl:text-3xl font-bold leading-tight sm:leading-normal text-gray-800 dark:text-white">{{ totalHours.toFixed(2) }}</p>
          </div>
    ...
    <div class="max-h-[500px] overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-sm">
      <table class="w-full">
        ...
        <thead class="bg-gray-50 dark:bg-gray-700 sticky top-0 z-10">
          <tr>
            <th class="py-3 text-center text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('employee') }}</th>
            <th class="py-3 text-center text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('type') }}</th>
            <th class="py-3 text-center text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('regular') }}</th>
            <th class="py-3 text-center text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('overtime') }}</th>
            <th class="py-3 text-center text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('sick') }}</th>
            <th class="py-3 text-center text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('pto') }}</th>
            <th class="py-3 text-center text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('holiday') }}</th>
            <th class="py-3 text-center text-[10px] font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider border-b dark:border-gray-600">{{ t('total') }}</th>
        ...
        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
          <!-- No Results Found -->
          <tr v-if="filteredAttendance.length === 0 && (searchQuery || filterDateFrom || filterDateTo)">
          ...
          <!-- Attendance Rows -->
          <tr
            v-for="(attendance, index) in filteredAttendance"
            :key="attendance.id"
          ...
          >
            ...
            <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-700 dark:text-gray-300 text-center">{{ getAttendanceType(attendance) }}</td>
            <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-700 dark:text-gray-300 text-center">{{ attendance.regular.toFixed(2) }} hrs</td>
            <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-700 dark:text-gray-300 text-center">{{ attendance.overtime.toFixed(2) }} hrs</td>
            <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-700 dark:text-gray-300 text-center">{{ attendance.sick.toFixed(2) }} hrs</td>
            <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-700 dark:text-gray-300 text-center">{{ attendance.pto.toFixed(2) }} hrs</td>
            <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-700 dark:text-gray-300 text-center">{{ attendance.holiday.toFixed(2) }} hrs</td>
            <td class="px-4 py-3 whitespace-nowrap text-xs font-semibold text-gray-900 dark:text-white text-center">{{ attendance.total.toFixed(2) }} hrs</td>
            ...
            <td class="px-4 py-3 whitespace-nowrap text-xs font-medium">
              <div class="relative">
                <button
                  @click="toggleActionMenu(attendance.id)"
                ...
                <!-- Dropdown Menu -->
                <div
                  v-if="activeActionMenu === attendance.id"
    ...
    <!-- Right Side Drawer for View/Edit Attendance -->
    <Transition name="drawer">
      ...
      >
        <div class="absolute right-0 top-0 h-full w-full max-w-4xl bg-white dark:bg-gray-800 shadow-xl overflow-y-auto">
          <div class="sticky top-0 bg-white dark:bg-gray-800 text-black dark:text-white px-6 py-5 flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between z-10 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center gap-3">
              ...
              <h2 class="text-xl font-bold text-black dark:text-white">
                {{ isViewMode ? t('viewAttendance') : editingAttendance ? t('editAttendance') : t('add') }}
              </h2>
            </div>
            <button
              @click="closeDrawer"
          ...
          <div v-if="selectedAttendance" class="p-6 space-y-6">
            ...
            <div class="bg-gray-50 dark:bg-gray-700 rounded-sm p-4">
              <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left justify-center sm:justify-between mb-4">
                <div class="flex items-center gap-3">
                  <div class="w-12 h-12 rounded-full flex items-center justify-center bg-blue-100 dark:bg-blue-900">
                    <img
                      v-if="selectedAttendance.employee && selectedAttendance.employee.profileImage"
                      :src="selectedAttendance.employee.profileImage"
                      :alt="selectedAttendance.employee.name"
                      class="w-full h-full object-cover rounded-full"
                    />
                    <span
                      v-else-if="selectedAttendance.employee"
                      class="text-blue-600 dark:text-blue-400 font-semibold"
                    >{{ selectedAttendance.employee.name.charAt(0).toUpperCase() }}</span>
                  </div>
                  <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ formatMonth(selectedAttendance.month) }}</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ getDateRange(selectedAttendance.month) }}</p>
                  </div>
                </div>
                <button
                  v-if="!isViewMode && selectedAttendance.status === 'Pending'"
                  @click="acceptAll"
              ...
              <div class="text-base sm:text-base md:text-xl lg:text-2xl xl:text-3xl font-bold leading-tight sm:leading-normal text-gray-900 dark:text-white mb-2">{{ selectedAttendance.total.toFixed(2) }} hrs</div>
              
              <div class="grid grid-cols-4 gap-4 mt-4">
                <div>
                  <div class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 uppercase">{{ t('regular') }}</div>
                  <div class="text-lg font-semibold text-gray-900 dark:text-white">{{ selectedAttendance.regular.toFixed(2) }} hrs</div>
                </div>
                <div>
                  <div class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 uppercase">{{ t('pto') }}</div>
                  <div class="text-lg font-semibold text-gray-900 dark:text-white">{{ selectedAttendance.pto.toFixed(2) }} hrs</div>
                </div>
                <div>
                  <div class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 uppercase">{{ t('overtime') }}</div>
                  <div class="text-lg font-semibold text-gray-900 dark:text-white">{{ selectedAttendance.overtime.toFixed(2) }} hrs</div>
                </div>
                <div>
                  <div class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 uppercase">{{ t('sick') }}</div>
                  <div class="text-lg font-semibold text-gray-900 dark:text-white">{{ selectedAttendance.sick.toFixed(2) }} hrs</div>
                </div>
              </div>
            </div>
            ...
            <!-- Time Entries Table -->
            <div>
              <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase mb-3">{{ t('timeEntries') }}</h4>
              <div class="border border-gray-200 dark:border-gray-700 rounded-sm overflow-hidden">
                <table class="w-full">
                  <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                      <th class="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">{{ t('date') }}</th>
                      <th class="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">{{ t('checkIn') }}</th>
                      <th class="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">{{ t('checkOut') }}</th>
                      <th class="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">{{ t('mealBreak') }}</th>
                      <th class="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">{{ t('workHrs') }}</th>
                      <th class="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">{{ t('overtime') }}</th>
                      <th class="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">{{ t('note') }}</th>
                      <th class="px-3 py-2 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">{{ t('status') }}</th>
                      <th v-if="!isViewMode" class="px-3 py-2 text-center text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">{{ t('action') }}</th>
                    </tr>
                  </thead>
                  <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    <tr v-if="timeEntries.length === 0">
                      <td colspan="9" class="px-3 py-4 text-center text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">
                        {{ t('noTimeEntries') }}
                      </td>
                    </tr>
                    <tr
                    ...
                    >
                      <td class="px-3 py-2 text-xs text-gray-900 dark:text-white">{{ formatDate(entry.date) }}</td>
                      <td class="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">
                        <input
                          v-if="!isViewMode"
                          v-model="entry.checkIn"
                          type="time"
                          class="w-full px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        />
                        <span v-else>{{ entry.checkIn || 'N/A' }}</span>
                      </td>
                      <td class="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">
                        <input
                          v-if="!isViewMode"
                          v-model="entry.checkOut"
                          type="time"
                          class="w-full px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                        />
                        <span v-else>{{ entry.checkOut || 'N/A' }}</span>
                      </td>
                      <td class="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">
                        <input
                          v-if="!isViewMode"
                          v-model="entry.mealBreak"
                          type="number"
                          step="0.5"
                          min="0"
                          class="w-full px-3 py-2  text-xs border border-gray-300 dark:border-gray-600 rounded-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white h-[37px]"
                          placeholder="0"
                        />
                        <span v-else>{{ entry.mealBreak || 0 }} hrs</span>
                      </td>
                      <td class="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">{{ entry.workHrs.toFixed(2) }} hrs</td>
                      <td class="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">{{ entry.overtime.toFixed(2) }} hrs</td>
                      <td class="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">
                        <input
                          v-if="!isViewMode"
                          v-model="entry.note"
                          ...
                          :placeholder="t('note')"
                        />
                        <span v-else>{{ entry.note || 'N/A' }}</span>
                      </td>
                      ...
                      <td v-if="!isViewMode" class="px-3 py-2 text-center">
                        <button
                          @click="addTimeEntry"
                          v-if="index === timeEntries.length - 1"
                          class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 text-xs h-[37px]"
                          :title="t('addEntry')"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div v-if="!isViewMode" class="flex gap-3 pt-4">
              ...
              >
                {{ editingAttendance ? t('update') : t('save') }}
              </button>
              <button
                @click="closeDrawer"
      ...
      >
        <div class="bg-white dark:bg-gray-800 rounded-sm shadow-xl p-6 max-w-md w-full mx-4">
          <div class="flex items-center gap-4 mb-4">
            ...
            <div class="flex-1">
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('deleteAttendance') }}</h3>
              <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ t('areYouSureDeleteAttendance') }}</p>
            </div>
          </div>
...
// Load attendance from JSON file or localStorage
const loadAttendance = async () => {
  try {
    const { loadDataFromJSON } = await import('../utils/dataLoader')
    const attendanceData = await loadDataFromJSON('../data/employee_attendance.json', 'employee_attendance_data')
    const employeesData = await loadDataFromJSON('../data/employees.json', 'employees_data')
    
    // Merge attendance with employee data
    attendance.value = attendanceData.map(att => {
      const employee = employeesData.find(emp => emp.id === att.employeeId)
      return {
        ...att,
...
const saveAttendance = () => {
  try {
    const dataToSave = attendance.value.map(att => {
      const { employee, ...rest } = att
      return rest
    })
    localStorage.setItem('employee_attendance_data', JSON.stringify(dataToSave))
  } catch (error) {
...
const approvedRecords = computed(() => {
  return attendance.value.filter(att => att.status === 'Approved').length
})

const pendingRecords = computed(() => {
  return attendance.value.filter(att => att.status === 'Pending').length
})

const totalHours = computed(() => {
  return attendance.value.reduce((sum, att) => sum + att.total, 0)
})

// Get attendance type based on hours
const getAttendanceType = (att) => {
  if (att.regular > 0 && att.overtime === 0 && att.sick === 0 && att.pto === 0 && att.holiday === 0) {
    return 'Regular'
  } else if (att.overtime > 0) {
    return 'Overtime'
  } else if (att.sick > 0) {
    return 'Sick'
  } else if (att.pto > 0) {
    return 'PTO'
  } else if (att.holiday > 0) {
    return 'Holiday'
  }
  return 'Mixed'
}
...
const getDateRange = (monthString) => {
  ...
  return `${firstDay.getMonth() + 1}/${firstDay.getDate()}/${firstDay.getFullYear()} - ${lastDay.getMonth() + 1}/${lastDay.getDate()}/${lastDay.getFullYear()}`
}
...
const calculateOvertime = (workHrs) => {
  return Math.max(0, workHrs - 8)
}

// Watch time entries for changes
watch(timeEntries, (entries) => {
  entries.forEach(entry => {
    entry.workHrs = calculateWorkHrs(entry)
    entry.overtime = calculateOvertime(entry.workHrs)
  })
}, { deep: true })
...
const toggleActionMenu = (attendanceId) => {
  activeActionMenu.value = activeActionMenu.value === attendanceId ? null : attendanceId
}
...
const loadTimeEntries = (att) => {
  ...
  // If no entries, add at least one
...
// Add time entry
const addTimeEntry = () => {
  const [year, month] = selectedAttendance.value.month.split('-')
  const lastDate = timeEntries.value[timeEntries.value.length - 1].date
...
const acceptAll = () => {
  ...
  selectedAttendance.value.status = 'Approved'
}
  ...
  try {
    await withLoading(async () => {
      if (attendanceToDelete.value) {
        const index = attendance.value.findIndex(att => att.id === attendanceToDelete.value.id)
        if (index !== -1) {
          attendance.value.splice(index, 1)
          saveAttendance()
          showDeleteDialog.value = false
          success(`${t('attendanceDeleted')}: ${t('attendanceDeletedSuccess')}`)
          attendanceToDelete.value = null
        }
      }
    }, 'Deleting attendance...')
  } catch (err) {
    handleError(err, { userMessage: 'Failed to delete attendance. Please try again.' })
  }
}
...
const handleSubmit = async () => {
  ...
  if (editingAttendance.value) {
    ...
    await withLoading(async () => {
      const newAttendance = {
        id: `ATT${String(attendance.value.length + 1).padStart(3, '0')}`,
        employeeId: selectedAttendance.value.employeeId,
        month: selectedAttendance.value.month,
        regular,
        overtime,
        sick,
        pto,
